...
terraform {
  backend "s3" {
    bucket = "Ogulcan-Erdag-project-kgb-terraform-state" // Bucket where to SAVE Terraform State
...
data "terraform_remote_state" "network" {
  backend = "s3"
  config = {
    bucket = "Ogulcan-Erdag-project-kgb-terraform-state" // Bucket from where to GET Terraform State
